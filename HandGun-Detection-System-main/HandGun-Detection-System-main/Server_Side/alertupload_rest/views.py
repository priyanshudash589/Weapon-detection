...
def send_email(serializer):
    ...
    pb = PushBullet("o.VmS847unH6oJsjLz9Gy6MpR2L5brYHoV")
    
    # Send the data by passing the main title
    # and text to be send
    push = pb.push_note("Intruder Warning",  prepare_alert_message(serializer))
...
def prepare_alert_message(serializer):
    ...
    # url = 'http://127.0.0.1:8000/med/' + uuid[2]

    return 'Weapon Detected! View alert at ' + f"http://127.0.0.1:8000{serializer.data['image']}"
